...
<head>
    <meta charset="UTF-8">
    <title>{{ __('ui.mail_subject_updated') }}</title>
</head>
...
<body style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #f9f9f9; color: #333; padding: 40px 20px;">
    ...
    <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; padding: 30px; border-radius: 10px; box-shadow: 0 0 10px rgba(0,0,0,0.05);">
        ...
        <h2 style="color: #bfa046; text-align: center;">{{ __('ui.mail_heading_updated') }}</h2>

        <p style="font-size: 16px;">
            {{ __('ui.mail_greeting') }} <strong>{{ $booking->guest_first_name }} {{ $booking->guest_last_name }}</strong>,
        </p>

        <p style="font-size: 16px;">
            {{ __('ui.mail_updated_intro') }}
        </p>
        ...
        <table cellpadding="6" cellspacing="0" border="0" style="margin: 20px 0; width: 100%; font-size: 16px;">
            ...
            <tr>
                <td><strong>{{ __('ui.mail_checkin_new') }}:</strong></td>
                <td>{{ \Carbon\Carbon::parse($booking->check_in)->format('d/m/Y') }}</td>
            </tr>
            <tr>
                <td><strong>{{ __('ui.mail_checkout_new') }}:</strong></td>
            ...
            <tr>
                <td><strong>{{ __('ui.mail_total_price_updated') }}:</strong></td>
        ...
        <p style="font-size: 16px;">{{ __('ui.mail_mod_cancel_text') }}</p>
